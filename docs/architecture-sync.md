...
B2 里、自动取回与手动恢复为什么语义不同、保留窗口怎么算、**游戏身份(指纹 + 配对)
怎么认"哪一条是本机这一款"、云端索引一份与它在本机的那份缓存**、以及凭据的存储与
挑选顺序。**什么时候读它**:改同步行为、改引擎、改凭据后端、改配对 / 身份 / 索引,
或者排查"存档没上传 / 传错了 / 恢复没生效"的时候。
...
| `rclone`(备选) | `<prefix>/games/<cloud_dir>/<stamp>.zip`,一版一个完整 zip(身份卡与包并排) | **不加密**,落桶即明文 | — |
...
├── games/<cloud_dir>/kotori-game.json        ← 这一款的身份卡(kind: identity),与包并排
├── index/kotori-index.json                   ← 云端索引:一个桶一份的并集快照
│   index/log/<machine_id>-<时间戳>.json       ← 每次写入附带的一条增量
```

rclone 那边,`games/` 下的目录名(`<cloud_dir>`)就是一个游戏;**身份卡与版本包放在
同一个目录下面** —— 身份卡是"云端这一款叫什么、指纹是谁"的答案,包是版本本体。
树里的 `index/` 是**云端的索引**,这一层只有索引,没有存档(下面那节「云端索引与
本机缓存」专门讲它)。kopia 那条路上**没有目录**:整个仓库落在 `<prefix>/kopia`
(`engine::repo_prefix`),一个游戏一版 = 一条 `kind:save`、`game:<cloud_id>` 的快照;
身份卡与索引也各是一条快照(`kind:identity` / `kind:index`,索引快照再靠
`index:main` 或 `index:<增量名>` 区分)。

- **一版一个包,整份上、整份下**。没有 `current/` + `versions/` 那一对目录了:
  "最新的一版"就是**名字最大的那个包**,不额外维护指针文件 —— 少一个会写坏的东西。
  "回到某一版"= 把那个包铺下去,不是"先铺最新的、再把当时的 diff 叠上去"。
- ⚠ **`cloud_dir`(云端落点)与 `cloud_id`(身份)是两件事**:两台机器给同一款游戏起的
  名字可以不同、本机的 id 也可以不同,但"版本放哪个目录"必须一致,否则另一台机器
  看不见这一台的版本。所以落点单独记(`GameConfig::cloud_dir`;缺省 = 本机游戏 id,
  也就是从前的行为),而"这一款在云端是谁"记在 `cloud_id`(见下一节)。
...
- **包内结构**(`archive/`):每个存档位置占一个顶层目录 `<save_key>/<相对路径>`,包根
  ...
  路径),否则恢复一个被改过的包时覆盖的就不只是存档了。**包里还带一段身份**
  (`PackIdentity`):取回与恢复都要拿它跟本机 `cloud_id` 比对,对不上就跳过/拒绝,
  绝不把一个身份的存档铺到另一个身份的游戏上。

kopia 那条路:**整个仓库**落在 `<prefix>/kopia`(`engine::repo_prefix`),一个游戏一版 =
一个快照,快照的 description 就是 `version_stamp`。恢复后的目录结构与 zip 里的
一模一样,`archive::plan` 的合并判定原样复用。列版本、算保留窗口都**只许看
`kind:save` 那一种快照**——仓库里混着身份与索引快照,不能把它们当成一版存档。

---

## 游戏身份:指纹与配对

云端只有两样东西:**存档包**与**身份卡**。本机的游戏库(路径、exe 位置、缩放档案)
**不上云**;两台机器要认得"我这一款"与"你那一款"是同一款,靠的是这一节的东西。

- **exe 指纹**(`src/sync/fingerprint.rs`)回答"是不是同一款":判据是文件大小 +
  首尾各 1 MiB 的 BLAKE2b-256(≤ 2 MiB 的文件整份读,不能切头尾 —— 那样中间那段
  会被数两遍)。字符串形如 `v1:<size>:<hex>`,**形状(算法、窗口)变了就换版本号**:
  老指纹从此对不上任何人(等于"还没有指纹"),而不是与新指纹碰巧相等。超过 2 MiB
  的文件中间改了认不出,这是"不做整文件哈希"的另一面,也是刻意的——指纹只当提议,
  命中与否只决定"要不要自动绑定",不决定"要不要覆盖存档"。读不到 exe(盘不在、
  没权限)就如实返回 `None`,**绝不编一个"大概"的值**。
- **配对**回答"本机哪一条档案 ↔ 云端哪一条身份"。结果落在 `GameConfig::cloud_id`
  (身份)与 `cloud_dir`(云端落点)上,**粘住**,只有用户能改(配对界面);
  指纹只当提议,改身份要用户点头。判错的代价是"把存档铺到错误的游戏上",所以
  规矩是**宁可不动,也不猜**(`pairing.rs`):指纹**唯一命中**才自动绑——而且那条
  身份在云端还得是"一个指纹只属于一条身份"、没被本机别的档案认领;命中多条、
  指纹在云端有歧义、或者只靠名字 / 存档位置"像"的,一律列出来问用户。
- 用户点过「不是同一款」会记进 `GameConfig::cloud_rejected`:下次配对与扫描看到
  它在名单里,就不再自动绑回来(那条自动规则唯一的刹车),连候选都不再列。
- 每个游戏还有自己的同步开关 `sync_enabled`(默认开;关掉**只影响自动同步** ——
  启动前不取回、退出后不上传;**手动**「立即同步」「取回存档」是明确的用户动作,
  不受它限制),以及「配对结论」`cloud_conclusion`:`ok:<目标签名>`(已确认) /
  `off:<目标签名>`(问过,用户选了"关掉这一款的同步");`None` = 未定。
  **目标签名** = `v1:<引擎>:<endpoint>:<bucket>:<prefix>`(`src/sync/signature.rs`,
  endpoint / bucket / prefix 都做归一化):换桶 / 换引擎 / 换 prefix 之后结论自动
  失效(回到"未定"),而改保留版本数、换凭据**不会**——那些与"云端那一条是谁"无关
  ("要不要重扫一遍"该由目标本身有没有变来决定)。
- **指纹只在三个时刻算**,**没有**"启动时把全库 exe 读一遍"那回事(读了也只是
  白花 IO,补上了也没人看):`game.create` 建档案时算一次;`game.update` 里换了 exe
  重算一次(**每次用户交上来都重算**,路径没变也要算:就地换了版本、打了补丁都该
  认出来;算不出就如实记成"还不知道",**绝不留下一次的答案** —— 一个过期的指纹
  会把这一款绑到别的云端身份上);配对 / 上传前**按需补齐**(`fill_missing`,幂等,
  只补缺的,已经有指纹的一条都不碰)。
- 身份卡(`kotori-game.json`,`kind: identity`)是云端这一款的"档案":`cloud_id` +
  给人看的名字 + 认领过它的机器列表(`MachineIdentity`:`machine_id` / 机器名 /
  指纹 / 存档位置 key / 用过的 exe 路径)。包清单里也带一段身份(`PackIdentity`),
  取回与恢复前拿它跟本机 `cloud_id` 比对,**不一致就绝不铺到本机存档上**。
- **启动游戏前的自检**(`sync::selfcheck`):目标没配齐 / 这一款开关关着 ⇒ 一个字
  都不做,直接启动;在**当前目标**上已确认(结论里的签名与当前签名一致)⇒ 不重扫、
  不问,照常取回(防错配闸照旧生效);未定 ⇒ 按指纹在**当前桶**里找一次,恰好命中
  一条且没被本机别的档案认领 ⇒ 静默认领并盖章;而且这种情况少,多半还是得**问一次**
  再起游戏(界面:启动前那一问)。**自检绝不拦启动**:读云端失败按"未定"处理,用户
  照样能开始玩。
...
"云端有哪些游戏、叫什么名字"不看每一张身份卡了(kopia 那边读一张卡 = 一次
`restore`,200 款就是 200 趟),改成**一个桶一份的索引**(`src/sync/index.rs`):

- **逻辑上一份,物理上是"合并快照 + 未合并增量"**:`index/kotori-index.json`
  (rclone 那边;kopia 是 `kind:index`、`index:main` 的那条快照)是**并集快照**,
  `index/log/<machine_id>-<时间戳>.json` 是每次写入附带的一条增量(名字唯一,
  谁也覆盖不了谁)。**写入顺序是先增量、再重写 main**:两台机器同时同步时,各自
  "读-改-写"同一份 main 会互相抹掉,有了增量,丢掉的那一条仍躺在 `log/` 里;
  崩在"写完增量、还没写 main"中间也一样不丢条目。读的时候把"main + main 里没
  记过的增量"合起来看。
- 索引里存的是**不常变**的东西:云端身份(名字、指纹、机器、用过的 exe 路径)+
  很轻的摘要(有几版、最近一版的名字与大小)。
- **身份卡才是真相,索引只是镜像**:索引丢了 / 坏了照着卡能重建(「深度扫描云端」
  就是干这个的),所以索引**读不懂时当"没有索引"处理**,绝不让它把人卡死;
  写失败也只记日志,**绝不许让上传失败** —— 它只是加速用的,包才是要紧的东西。
- **读的是本机缓存**(`src/sync/index_cache.rs`):读云端索引本身也是一次网络往返
  (rclone 要 cat 两个对象,kopia 要列快照再 restore),所以拿到一次就落本机:
  缓存放在数据目录(`KOTORI_DATA_DIR`,默认 `~/.local/share/kotori`)下的
  `cloud-index/<目标签名哈希>.json`,内容 `{格式, 签名, 拿到的时间, 索引}`,先写
  临时文件再改名(崩在半路不留下半截 JSON)。**按目标签名分开存**:换桶之后绝不会
  读到旧桶的清单(那会把 A 桶的身份绑到 B 桶的档案上);读不懂 / 格式不认识 /
  签名对不上 —— 一律当**没有缓存**,绝不猜(代价只是白跑一趟网络,而猜错的代价
  是一条错的绑定)。
- 读路径(「云端存档」页进入、「自己选…」浮层、添加页的匹配)**默认只读本地**;
  缓存超过**一小时**(`CACHE_TTL`)、或者本地还没有缓存时才会真的去云端,并顺手
  把缓存刷新。「云端存档」页那颗**刷新**按钮是唯一强制联网的读。
- 后台还有两条:daemon 起来先刷一次、之后**每小时**刷一次(和"缓存多旧算旧"是
  同一个数);改完**同步设置**也刷一次(目标签名会变——换桶、换引擎、换 prefix
  才是;只改了保留版本数、程序位置那种与目标无关的字段不刷,也不阻塞那一次保存)。
  上传成功与「深度扫描云端」之后顺手更新缓存(那时刚知道最新内容)。
- 刷新失败时如果手上有旧缓存就**照用**,并把失败原因带给界面(界面上会写
  "本机缓存 · 什么时候拿到的",旁边补一句为什么它旧了)。
- ⚠ 索引里记的**用过的 exe 路径**只有两个用途:给人看、当搜索关键字。它**不参与
  任何判断**(认人只看指纹)、**不写回本机配置**、也**不拿来还原本机目录**。

---

## 3. rclone 引擎:一版一个 zip

版本包走 `copyto`(上传一个对象)/ `deletefile`(删掉一个对象)/ `lsf --files-only`
(列目录)三种调用;身份卡与索引另走 `cat`(读一个 json 对象)与 `mkdir`(幂等建前缀
目录),不存在"目录对目录地合并"。
...
- **云端版本看得到,但删不下**:「云端存档」页能看到每一款每一版(版本名、大小、
  时间,按**云端落点**列,走 `sync.cloud_versions`);但**删除某一版、下载 / 导出某一版
  还没有界面**(后端 `sync.cloud_versions` / `sync.restore` 在)。单游戏页也仍然只有
  「恢复」(恢复最新那一份),没有版本选择器。**存档位置覆盖也极少**:绝大多数游戏
  没有配 `save_paths`,同步实际上没有东西可传。
- **每个游戏的同步开关(`sync_enabled`)目前只能在"启动前那一问"里关**,还没有单独的
  界面入口(手动同步不受它限制,见「游戏身份」一节)。
- **Windows 那一轮还没跑**:keyring 后端未实现(见第 8 节),IPC、单实例锁、UI 只验过
  移植本身,完整的 Windows 同步往返没有实测。
