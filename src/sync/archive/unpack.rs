...
pub(super) fn extract_with(
    ...
    // 这里按 zip 的成员逐个写出，**不看清单** —— 清单之外的成员因此也会落到
    // `into` 里。这是**刻意的**，不是漏掉的检查（GAP-5，2026-09-25 定：不修，
    // 只把口径写在这里）：
    //   * 谁会产生多余成员 —— kotori 自己打的包不会：`pack` 写进去的只有存档
    //     文件与清单本身，两者出自同一份列表。真出现多余成员只有两种可能：桶被
    //     别人改过，或者将来的版本往包根多放了一个文件（跨版本兼容）。
    //   * 它有什么后果 —— 没有。铺回存档目录的那一步（[`plan`]）只遍历
    //     `manifest.entries`，清单外的东西没有资格进用户的存档；它们只在临时
    //     目录里躺一下，跟着临时目录一起消失。撑不爆磁盘由上面的 [`Limits`] 兜着。
    //   * 为什么不改成"发现清单外成员就拒绝" —— 清单文件自己就**不在** entries
    //     里（`pack` 把 [`MANIFEST`] 单独 `start_file`），这条规则第一件事就得给
    //     自己开一个特例，反而更容易写错；而它拦下来的东西本来也无害。
