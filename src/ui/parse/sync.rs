...
/// `sync.pairing` 的回包：云端身份 ↔ 本机档案。
///
/// 状态由 daemon 直接给（`state` 用网页里那种数字），这里只把它翻成模型 —— 界面不该
/// 自己去猜"这一行为什么是黄的"。
pub(in crate::ui) fn parse_pairing(value: &Value) -> Result<Vec<PairingRow>, String> {
    let rows = value
        .get("rows")
        .and_then(Value::as_array)
        .ok_or_else(|| "回包里没有 rows".to_string())?;
    ...
    rows.iter()
        .map(|row| {
            ...
            let state = match row.get("state").and_then(Value::as_u64) {
                Some(1) => PairingState::AutoBound,
                Some(2) => PairingState::Ask,
                Some(3) => PairingState::Missing,
                _ => PairingState::Bound,
            };
            ...
            Ok(PairingRow {
                ...
                cloud_name: text("cloud_name"),
                machines: row.get("machines").and_then(Value::as_u64).unwrap_or(0),
                state,
...
mod tests {
    #[test]
    fn pairing_rows_come_back_with_their_state_and_candidates() {
        let payload = serde_json::json!({
            "rows": [
                {
                    "cloud_key": "original-name", "cloud_id": "8f2c1234-0000-0000-0000-000000000000",
                    "cloud_name": "Original Name", "machines": 2, "state": 1,
                    "local_id": "renamed", "local_name": "Renamed", "evidence": "fingerprint",
                    "choices": [],
                },
                {
                    "cloud_key": "other", "cloud_id": "1111",
                    "cloud_name": "Other", "machines": 1, "state": 2,
                    "local_id": "", "local_name": "", "evidence": "",
                    "choices": [{ "local_id": "a", "local_name": "甲" }, { "local_id": "b", "local_name": "乙" }],
                },
            ],
            "bound": 1,
        });
        let rows = parse_pairing(&payload).unwrap();
        assert_eq!(rows.len(), 2);
        assert_eq!(rows[0].state, PairingState::AutoBound);
        assert_eq!(rows[0].local_name, "Renamed");
        assert!(rows[0].choices.is_empty());
        assert_eq!(rows[1].state, PairingState::Ask);
        assert_eq!(rows[1].choices.len(), 2);
        assert_eq!(rows[1].choices[1].1, "乙");
