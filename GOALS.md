...
- 密码**由用户自己设定，我们绝不生成**；磁盘上没有明文，用户随时能自己取回来
- **凭据三级存储，只有第一级是平台相关的**（ADR-014）：
  1. 系统凭据库 —— Linux Secret Service / Windows 凭据管理器（待实现）/ macOS Keychain（未支持）
  2. **主密码加密文件** —— Argon2id + ChaCha20-Poly1305，跨平台同一份代码；
     没有密钥环的设备（niri 会话、容器、Windows 首次可用形态）靠它把凭据加密常驻，
     开机输一次主密码即可，而不是每次重填 B2 key
  3. 仅本次会话（内存）—— 最后手段，明确告知用户
